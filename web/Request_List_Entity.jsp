    ...
    <center>
        <div style="width: 960px;height: auto;background-color: white;border: 2px solid whitesmoke;box-shadow:0 0 36px #000;-moz-box-shadow:0 0 36px #000;-webkit-box-shadow:0 0 6px #000;">
            ...
            <div style="width: 960px;height: 400px;overflow-y:  scroll;background-color: white;color:black;border-bottom: 4px solid #99FF00">
                ...
                <center>  ${in.msg} 
                    <h3><a style="text-decoration: none"><%="" + u.getName()+"'s Services Requests"%></a></h3>
                    <table border="1" class="table table-striped" id="sample_1" style="">
                        <thead>
                            <tr>
                                <td style="background-color: #3d3d3d;color: white">No</td>
                                
                                <td style="background-color: #3d3d3d;color: white">Requesting Date</td>
                                 <td style="background-color: #3d3d3d;color: white">Visitor</td>
                                <td style="background-color: #3d3d3d;color: white">Service</td>
                                <td style="background-color: #3d3d3d;color: white">Details</td>
                                <td style="background-color: #3d3d3d;color: white">Request Status</td>
<!--                                <td style="background-color: #3d3d3d;color: white">Reject</td>
                                <td style="background-color: #3d3d3d;color: white">Accept</td>-->
                            </tr>
                        </thead>
                        <tbody>
                            <%
                                List<Service_Request> l = Service_Request.getList();
                                int no = 1;
                                for (int i = 0; i < l.size(); i++) {
                                   
                                    Service sr = Service.getPublic_ServiceFromID(l.get(i).getService_id() + "");
                                     if(sr.getEntity_id()==u.getInstutition_id()){
                                    Visitor in = Visitor.getVisitor_ID(l.get(i).getVisit_id()+"");
                            %>
                            <tr>
                                <td><%=no%></td>
                                   <td><%=l.get(i).getRequest_date()%></td>
                                <td><%=in.getNames()  %></td>
                                <td><%=sr.getName()%></td>
                            
                                <td><%=l.get(i).getDetails()%></td>
                                 <%
                                    if(l.get(i).getRequest_status().equalsIgnoreCase("pending")){
                                    %>
                                    <td style="background-color: yellow"><%=l.get(i).getRequest_status()%></td>
                                    <%
                                    }else{
                                     if(l.get(i).getRequest_status().equalsIgnoreCase("rejected")){
                                        %>
                                    <td style="background-color: red;color: white"><%=l.get(i).getRequest_status()%></td>
                                    <%
                                     }else{
                                      %>
                                    <td style="background-color: green;color: white"><%=l.get(i).getRequest_status()%></td>
                                    <%
                                     }
                                    }
                                    %>
<!--                                    <td><a href="Option_Processing.jsp?idx=<%=l.get(i).getRequest_id()%>&op=Rejected">Reject</a></td>
                                 <td><a href="Option_Processing.jsp?idx=<%=l.get(i).getRequest_id()%>&op=Accepted">Accept</a></td>
                            -->
                            </tr>
                            <%
                                    no++;
