...
<%--    
    ...
    if (u == null) {
        dd.setMsg("<h3 style=color:red>Please Provide Your Valid Username and Password</h3>");
%>
...
<html>
        ...
        <div style="width: 960px;height: auto;background-color: white;border: 2px solid whitesmoke;box-shadow:0 0 36px #000;-moz-box-shadow:0 0 36px #000;-webkit-box-shadow:0 0 6px #000;">
            ...
            <div style="width: 960px;height: 400px;overflow-y:  scroll;background-color: white;color:black;border-bottom: 4px solid #99FF00">
                ...
                <center>  
                    ${in.msg} 
                    <%
    Instutition n=Instutition.getInstutitionFromID(request.getParameter("id"));
    %>
    <%--
    String h="";
    ...
    Visit vv=Visit.getVisitFromID_Date(v.getCitizen_id()+"", fdate, n.getInstutition_id()+"");
     System.out.println(" Visitor id:"+v.getCitizen_id()+" Entity id:"+" "+n.getInstutition_id()+" Date:"+fdate);
    if(vv==null){
       vv=new Visit();
       vv.setEntity_id(n.getInstutition_id());
       vv.setVisitor_id(v.getCitizen_id());
       vv.insert();
    }
                    --%>
    <h3><a style="text-decoration: none;"><%=n.getName()+"'s Delivered Service(s)" %></a></h3>
                    <table border="1" class="table table-striped" id="sample_1" style="">
                        <thead>
                            <tr>
                                <td style="background-color: #3d3d3d;color: white">No</td>
                                <td style="background-color: #3d3d3d;color: white">Service Name</td>
                                <td style="background-color: #3d3d3d;color: white">Description</td>

                                <td style="background-color: #3d3d3d;color: white">Cost</td>
                                <td style="background-color: #3d3d3d;color: white">Duration</td>
                                 <td style="background-color: #3d3d3d;color: white">Option</td>
                            </tr>
                        </thead>
                        <tbody>
                            <%
                                List<Service>l=Service.getList();
                                int no=1;
                                for(int i=0;i<l.size();i++){
if(l.get(i).getEntity_id()==Integer.parseInt(request.getParameter("id"))){  %>
                            <tr>
                                <td><%=no %></td>

                                <td><%=l.get(i).getName()%></td>
                                <td><%=l.get(i).getDescription()%></td>
                                <td><%=l.get(i).getCost()+""%></td>
                                <td><%=l.get(i).getDuration()+" Day(s)"%></td>
                                <td><a href="Request_Form.jsp?ix=<%=l.get(i).getService_id()+""%>&id=<%=request.getParameter("id")+""%>">Request it Now</a></td>
                            </tr>
                            <% 
no++;   }                              
}%>
